...
function init_twitter_carusel() {
    // The theme's server-side Twitter proxy is not part of this application.
    return;
    if ($('.twitter-1').length) {
        $('.twitter-1').tweet({
...
/********   Carousel   *********/
/**/
var owl_single = $('.owl-two-pag:not(.eg-static-grid)')
jQuery(owl_single).each(function () {
    jQuery(this).owlCarousel({
        itemsCustom: [
            [0, 1],
...
function init_rev_slider() {
    // The legacy Revolution Slider was replaced by the responsive .eg-hero.
    // Do not initialize a hidden zero-width slider: TweenMax receives NaN sizes.
    if ($('.eg-hero').length) {
        return;
    }
